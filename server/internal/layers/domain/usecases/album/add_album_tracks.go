...
type AddAlbumTracksParams struct {
	Id       int
	TrackIds []int
}

func (u *AlbumUsecase) AddAlbumTracks(
	ctx context.Context,
	params AddAlbumTracksParams,
	...
	if err := u.albumRepository.AddAlbumTracks(album); err != nil {
		logger.MainLogger.Error("Couldn't add album tracks", err, album)
		return nil, entities.NewInternalError(errors.New("Failed to add album tracks"))
	}
