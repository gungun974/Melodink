...
data class PlayerStatus (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): PlayerStatus {
      val playing = pigeonVar_list[0] as Boolean
      val pos = pigeonVar_list[1] as Long
      val positionMs = pigeonVar_list[2] as Long
      val bufferedPositionMs = pigeonVar_list[3] as Long
      val state = pigeonVar_list[4] as MelodinkHostPlayerProcessingState
      val loop = pigeonVar_list[5] as MelodinkHostPlayerLoopMode
    ...
    return when (type) {
      129.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          MelodinkHostPlayerProcessingState.ofRaw(it.toInt())
        }
      }
      130.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          MelodinkHostPlayerLoopMode.ofRaw(it.toInt())
        }
      }
      131.toByte() -> {
  ...
  fun updateState(stateArg: MelodinkHostPlayerProcessingState, callback: (Result<Unit>) -> Unit)
{
  ...
  fun externalPause(callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.pigeon_melodink.MelodinkHostPlayerApiInfo.externalPause$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(null) {
      if (it is List<*>) {
        if (it.size > 1) {
