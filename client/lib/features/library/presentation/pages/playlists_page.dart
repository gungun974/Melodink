...
class PlaylistsPage extends HookWidget {
  const PlaylistsPage({super.key});

  @override
  Widget build(BuildContext context) {
    useEffect(() {
      final loadPlaylists = context.read<PlaylistsViewModel>().loadPlaylists;
      Future(() {
        loadPlaylists();
      });
      return null;
                    ...
                    if (size == AppScreenTypeLayout.mobile)
                      AppIconButton(
                        icon: Container(
                          ...
                          child: Center(
                            child: AdwaitaIcon(size: 20, AdwaitaIcons.list_add),
                          ),
                        ),
