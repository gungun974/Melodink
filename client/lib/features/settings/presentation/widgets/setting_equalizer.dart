...
class SettingEqualizer extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final bands = [
      useState(settings?.equalizer.bands[60] ?? 0),
      useState(settings?.equalizer.bands[150] ?? 0),
      useState(settings?.equalizer.bands[400] ?? 0),
      useState(settings?.equalizer.bands[1000] ?? 0),
      useState(settings?.equalizer.bands[2400] ?? 0),
      useState(settings?.equalizer.bands[15000] ?? 0)
    ];

    ref.listen(appSettingsNotifierProvider, (_, asyncSettings) {
