...
static FlValue* pigeon_melodink_player_status_to_list(PigeonMelodinkPlayerStatus* self) {
  ...
  fl_value_append_take(values, fl_value_new_custom(130, fl_value_new_int(self->state), (GDestroyNotify)fl_value_unref));
  fl_value_append_take(values, fl_value_new_custom(131, fl_value_new_int(self->loop), (GDestroyNotify)fl_value_unref));
  return values;
}
...
void pigeon_melodink_melodink_host_player_api_info_update_state(PigeonMelodinkMelodinkHostPlayerApiInfo* self, PigeonMelodinkMelodinkHostPlayerProcessingState state, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data) {
  g_autoptr(FlValue) args = fl_value_new_list();
  fl_value_append_take(args, fl_value_new_custom(130, fl_value_new_int(state), (GDestroyNotify)fl_value_unref));
